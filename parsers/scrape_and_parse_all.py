...
def scrape_courses() -> list:
    ...
    while True:
        url = (
            f"https://www.unibo.it/it/didattica/corsi-di-studio/elenco?schede={scheda}"
        )
    ...
    for i in tqdm(range(scheda)):
        ...
        for item_area in item_areas:
            ...
            try:
                ...
                course["campus"] = (
                    campus_.replace("Sede didattica: ", "").strip().split(",")
                )
            ...
            for y in years:
                ...
                if r.status_code != 200:
                    ...
                    for i in teachings:
                        if (
                            last
                            and "modulo" in i.get("class", [])
                            and "modulo" not in last.get("class", [])
                        ):
